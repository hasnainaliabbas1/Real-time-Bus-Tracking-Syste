...
export function BusCard({ bus, onTrackClick }: BusCardProps) {
  const [status, setStatus] = useState<{ type: string; label: string; }>({ 
    type: "on-time", 
    label: "On Time" 
  });
  const [eta, setEta] = useState<{ minutes: number; delayed?: number }>({ minutes: 0 });
  ...
  useEffect(() => {
    ...
    const delayedBy = isDelayed ? Math.floor(Math.random() * 10) + 1 : undefined;
